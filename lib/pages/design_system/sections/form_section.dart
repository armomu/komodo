...
class _TextFieldDemo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _FormCategory(
      ...
      child: Column(
        children: [
          const TextField(decoration: InputDecoration(hintText: '请输入文字', prefixIcon: Icon(Icons.search))),
          const SizedBox(height: 12),
          const TextField(decoration: InputDecoration(labelText: '用户名', prefixIcon: Icon(Icons.person))),
          const SizedBox(height: 12),
          TextField(
            decoration: const InputDecoration(labelText: '邮箱', prefixIcon: Icon(Icons.email), helperText: '请输入有效的邮箱地址'),
            keyboardType: TextInputType.emailAddress,
          ),
          const SizedBox(height: 12),
          TextField(
            decoration: const InputDecoration(labelText: '密码', prefixIcon: Icon(Icons.lock), errorText: '密码不能为空'),
            obscureText: true,
          ),
          const SizedBox(height: 12),
          const TextField(
            decoration: InputDecoration(labelText: '电话号码', prefixIcon: Icon(Icons.phone), filled: true),
            keyboardType: TextInputType.phone,
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(child: TextField(decoration: const InputDecoration(labelText: '验证码', prefixIcon: Icon(Icons.verified_user)))),
      ...
      child: Column(
        children: [
          ...
          Row(
            ...
            children: [
              const Text('深色模式'),
              Switch(value: _switch2Value, onChanged: (v) => setState(() => _switch2Value = v)),
            ],
          ),
          const Divider(),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('禁用状态', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
      ...
      child: Column(
        children: [
          ...
          Row(
            ...
            children: [
              const Text('不确定状态'),
              Checkbox(value: _tristateValue, tristate: true, onChanged: (v) => setState(() => _tristateValue = v)),
            ],
          ),
          const Divider(),
          Row(
      ...
      child: Column(
        children: [
          ...
          RadioListTile<int>(
            ...
            groupValue: null,
            onChanged: null,
            title: Text('禁用状态', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
          ),
        ],
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          RangeSlider(
            ...
            labels: RangeLabels('${(_rangeValues.start * 100).toInt()}', '${(_rangeValues.end * 100).toInt()}'),
          ),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('${(_rangeValues.start * 100).toInt()}', style: Theme.of(context).textTheme.bodySmall),
              Text('${(_rangeValues.end * 100).toInt()}', style: Theme.of(context).textTheme.bodySmall),
            ],
          ),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    return _FormCategory(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('基础 Chip', style: TextStyle(fontSize: 12, color: Colors.grey)),
          ...
          Wrap(
            ...
            children: [
              ...
              Chip(label: const Text('音乐'), avatar: const Icon(Icons.music_note, size: 18)),
              Chip(label: const Text('可删除'), onDeleted: () => _showSnackBar(context, '删除标签'), deleteIcon: const Icon(Icons.close, size: 18)),
            ],
          ),
          const SizedBox(height: 16),
          const Text('FilterChip (可选择)', style: TextStyle(fontSize: 12, color: Colors.grey)),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              FilterChip(label: const Text('未选中'), selected: false, onSelected: (v) => _showSnackBar(context, '选择: $v')),
              FilterChip(label: const Text('已选中'), selected: true, onSelected: (v) => _showSnackBar(context, '选择: $v')),
              FilterChip(label: const Text('禁用'), selected: false, onSelected: null),
            ],
          ),
          const SizedBox(height: 16),
          const Text('InputChip (输入型)', style: TextStyle(fontSize: 12, color: Colors.grey)),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              ...
              InputChip(label: const Text('带头像'), avatar: const CircleAvatar(child: Text('A')), onPressed: () {}),
              InputChip(label: const Text('可删除'), onDeleted: () {}, deleteIcon: const Icon(Icons.close, size: 16)),
            ],
          ),
          const SizedBox(height: 16),
          const Text('ActionChip (操作型)', style: TextStyle(fontSize: 12, color: Colors.grey)),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              ActionChip(label: const Text('添加'), avatar: const Icon(Icons.add, size: 18), onPressed: () => _showSnackBar(context, '添加')),
              ActionChip(label: const Text('编辑'), avatar: const Icon(Icons.edit, size: 18), onPressed: () => _showSnackBar(context, '编辑')),
              ActionChip(label: const Text('删除'), avatar: const Icon(Icons.delete, size: 18), onPressed: () => _showSnackBar(context, '删除')),
            ],
          ),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    return _FormCategory(
      ...
      child: Wrap(
        ...
        runSpacing: 12,
        children: [
          ElevatedButton.icon(onPressed: () => _showDatePicker(context), icon: const Icon(Icons.calendar_today), label: const Text('选择日期')),
          ElevatedButton.icon(onPressed: () => _showTimePicker(context), icon: const Icon(Icons.access_time), label: const Text('选择时间')),
        ],
      ),
    );
  }
